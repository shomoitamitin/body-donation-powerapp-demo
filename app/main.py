...
def get_donors():
    ...
    donors = [dict(row._mapping) for row in result]
    db.close()
...
def get_donor_next_of_kin(donor_id: int):
    ...
    next_of_kin = [dict(row._mapping) for row in result]
    db.close()
...
def get_funeral_homes():
    ...
    funeral_homes = [dict(row._mapping) for row in result]
    db.close()
...
def get_donor_profile(donor_id: int):
    ...
    medical_history = medical_result.fetchone()
    ...
    profile = {
        ...
        "medical_history": dict(medical_history._mapping) if medical_history else None,
